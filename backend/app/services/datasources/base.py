...
class VacancyParser(ABC):
    ...
    async def parse_and_save(
        ...
        try:
            ...
            async for vacancy in self.search_vacancies(filters, max_results):
                if vacancy.external_id not in self.seen_vacancy_ids:
                    ...
                    if len(vacancies) % 100 == 0:
                        self.logger.info(f"Collected {len(vacancies)} vacancies...")
            ...
            self.logger.info(
                f"Parsing completed: {len(vacancies)} vacancies saved to {output_file}"
            )
    ...
    async def _save_vacancies_to_json(self, vacancies: List[Dict], filename: str):
