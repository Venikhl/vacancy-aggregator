        ...
        <Popover open={open} onOpenChange={setOpen} modal={modal}>
            <PopoverTrigger asChild>
                {renderTrigger ? (
                    ...
                    <div
                        className={cn(
                            'text-primary bg-foreground flex w-full cursor-pointer items-center h-9 ps-3 pe-1 font-normal border border-input rounded-md text-sm shadow-sm',
...
function TimePicker({
    ...
    const [ampm, setAmpm] = useState(
        format(value, 'a', { locale: ru }) === 'AM' ? AM_VALUE : PM_VALUE,
    );
    const [hour, setHour] = useState(
        use12HourFormat
            ? +format(value, 'hh', { locale: ru })
            : value.getHours(),
    );
...
function buildTime(options: BuildTimeOptions) {
    ...
    let date: Date;
    if (use12HourFormat) {
        const dateStrRaw = format(value, formatStr, { locale: ru });
